...
\title{A Direct Proof of Liu's Entropy Bound for the Union-Closed Sets Conjecture}
\author{K\aa re}
\date{}
...
\end{equation}
for Frankl's union-closed sets conjecture.  This constant is the one obtained by Liu through a conditionally independent coupling, subject to numerical structural hypotheses.  We give a direct certified proof of the underlying bivariate entropy inequality.  The argument combines a Boppana-style Rolle method on the diagonal, a degree-27 Bernstein positivity certificate, and a finite Taylor-model interval-arithmetic verification of the off-diagonal critical-point structure.  As a consequence, every finite nonempty union-closed family different from \(\{\varnothing\}\) has an element contained in at least a \(c'\)-fraction of its members.  The code implementing all certificates is available at the repository listed in the appendix.
\end{abstract}
...
original form, we bypass them by proving directly the bivariate entropy
inequality needed for the \(0.382709\ldots\) bound.  This bivariate inequality
acts as a dual certificate for the relevant coupling.  Let
\begin{equation}
  I(x,y)=xy(1+(1-x)(1-y)).
\end{equation}
...
\end{equation}
The inequality \eqref{eq:intro_main_ineq} directly implies that every finite
nonempty union-closed family different from \(\{\varnothing\}\) has an element
belonging to at least a
\begin{equation}
  0.382709087918735\ldots
\end{equation}
...
\section{The union-closed corollary}\label{sec:corollary}

\begin{corollary}\label{cor:unionclosed}
Let \(\Fcal\subseteq2^{[n]}\) be finite, nonempty, union-closed, and not equal to \(\{\varnothing\}\).  Then some element belongs to at least
\begin{equation}\label{eq:cstar_cor}
  c'=1-\mstar=0.382709087918735\ldots
\end{equation}
of the sets in \(\Fcal\).
\end{corollary}

\begin{proof}
Let \(A\) be uniform on \(\Fcal\).  For a set \(S\), write
...
\begin{equation}\label{eq:absence_mean}
  \Prob[i\notin A]>1-c'=\mstar
\end{equation}
...
\begin{equation}\label{eq:Xi_def}
  X_i=\Prob[Z_i(A)=1\mid Z_1(A),\ldots,Z_{i-1}(A)].
\end{equation}
Then the chain rule gives
\begin{equation}\label{eq:chain_A}
  H(A)=\sum_{i=1}^n \E h(X_i),
\end{equation}
and
...
\begin{align}\label{eq:I_coupling}
  \Prob[U=1,V=1]&=I(x,y),\notag\\
  \Prob[U=1,V=0]&=x-I(x,y),\notag\\
  \Prob[U=0,V=1]&=y-I(x,y),\notag\\
  \Prob[U=0,V=0]&=1-x-y+I(x,y).
\end{align}
This is valid.  Indeed, \(I(x,y)\le x\) is equivalent to
\begin{equation}\label{eq:I_le_x}
  y(1+(1-x)(1-y))\le1,
\end{equation}
which follows from
\begin{equation}\label{eq:I_le_x_diff}
  1-y(1+(1-x)(1-y))=(1-y)(x+(1-x)(1-y))\ge0.
\end{equation}
Similarly \(I(x,y)\le y\).  Also,
\begin{equation}\label{eq:Frechet}
  I(x,y)\ge x+y-1,
\end{equation}
because, with \(a=1-x\) and \(b=1-y\), the difference is
\begin{equation}\label{eq:Frechet_diff}
  (1-a)(1-b)(1+ab)-(1-a-b)=ab(2-a-b+ab)\ge0.
\end{equation}
Thus \eqref{eq:I_coupling} has nonnegative entries and the correct marginals.

Implementing these couplings sequentially gives random sets whose marginals are uniform on \(\Fcal\).  Let the corresponding unions be denoted by \(A\cup B\) for the independent coupling and \(A\cup C\) for the \(I\)-coupling.  The chain rule and conditioning yield
\begin{align}\label{eq:entropy_union_lower}
  H(A\cup B)&\ge \sum_i \E h(X_iY_i),\notag\\
  H(A\cup C)&\ge \sum_i \E h(I(X_i,Y_i)),
\end{align}
where \(X_i,Y_i\) have the same one-dimensional law as \(X_i\).  Applying Theorem \ref{thm:bivariate} pointwise and averaging,
\begin{equation}\label{eq:entropy_pointwise_avg}
  \E\bigl((1-\bstar)h(X_iY_i)+\bstar h(I(X_i,Y_i))\bigr)
  \ge
  \frac{1}{\mstar}\E\frac{X_i h(Y_i)+Y_i h(X_i)}2.
\end{equation}
The right-hand side equals
\begin{equation}\label{eq:factor_avg}
  \frac{\E X_i}{\mstar}\E h(X_i)>\E h(X_i),
\end{equation}
by \eqref{eq:EXi}.  Summing over \(i\) gives
\begin{equation}\label{eq:strict_entropy}
  (1-\bstar)H(A\cup B)+\bstar H(A\cup C)>H(A).
\end{equation}
But \(\Fcal\) is union-closed, so both unions lie in \(\Fcal\).  Since \(A\) is uniform,
\begin{equation}\label{eq:entropy_upper_union}
  H(A\cup B)\le\log|\Fcal|=H(A),
  \qquad
  H(A\cup C)\le H(A).
\end{equation}
This contradicts \eqref{eq:strict_entropy}.  Therefore some element belongs to at least a \(c'\)-fraction of the sets.
\end{proof}
